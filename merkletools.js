...
var MerkleTools = function (treeOptions) {
    ...
    if (treeOptions) { // if tree options were supplied, then process them
        ...
        progressCB = treeOptions.progressCB;
        progressInterval = treeOptions.progressInterval;
    }
    ...
    //Get a prepared tree to save them.
    this.getPreparedTree = function () {
        const preparedTree = [];
    ...
    this.loadPreparedTree = function (levels) {
        tree.levels = [];
    ...
    let lastUpdatedTime;
    const startProgressCounter = function (num_leaves) {
        progress = 0;
        totalNodes = num_leaves % 2 !== 0 ? (num_leaves * 2) : (num_leaves * 2) - 1;
        lastUpdatedTime = Date.now();
    }

    const shouldUpdateProgress = function () {
        return (Date.now() - lastUpdatedTime) > progressInterval;
    }
    ...
    const incrementProgress = function () {
        completedNodes++;
        if (shouldUpdateProgress() && progressCB) {
    ...
    this.makeTree = function (doubleHash) {
        ...
        startProgressCounter(leafCount)
